...
public class MonthlyOpenClose {

	private Map<String, CloseOpenDto> monthlyOpenCloseMap = null;

	public void populateOpenClosePriceMap(String key, HistoricalDataDto historicalDataDto, Date date) {
		CloseOpenDto ocd = null;
		if (monthlyOpenCloseMap == null)
			monthlyOpenCloseMap = new HashMap<>();
		if (monthlyOpenCloseMap.containsKey(key)) {
			ocd = monthlyOpenCloseMap.get(key);
			...
			monthlyOpenCloseMap.put(key, ocd);
		}
	}

	public Map<String, CloseOpenDto> getMap() {
		return monthlyOpenCloseMap;
	}
